...
*   **Points:** Smaller tables radiating outwards, containing descriptions.
...
Contains the **context** (Who, What, Where, When).
...
## Part 2: Power BI Implementation (The VertiPaq Reality)
...
*   **Snowflake:** A Dimension filters another Dimension, which filters the Fact (e.g., `Product Category` -> `Product` -> `Sales`).
    ...
    *   **Power BI Reality:** **Bad.**
        *   It forces the engine to traverse multiple relationships (hops) to get to the data.
...
*   **Numbers Only:** Ideally, a Fact table contains *only* Keys and Numbers.
    *   *Exception:* If a text column has extremely high cardinality and is unique to the transaction (e.g., "Transaction Comment"), it can stay in the Fact, but it will bloat the file.
...
**"Push logic upstream."**
If you are writing complex DAX to bridge tables or clean up data, you have a modeling problem.
