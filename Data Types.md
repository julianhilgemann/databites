...
Power BI loads data into the **VertiPaq Engine**. It is highly optimized for compression. The data types here define how the engine stores column structures and how measures calculate results.
...
| **Decimal Number** | Floating Point (Double Precision). Can vary in precision. | Same as Whole Number. | **Use with Caution.**<br>Can result in tiny math errors (epsilon). High cardinality (many unique values) reduces compression significantly. |
...
| **Date** | Mapped to an Integer internally (Days since 1899-12-30). | Date functions return `BLANK` if input is invalid. | **Split Date & Time.**<br>Don't use `DateTime` in one column. Split into a `Date` column (for relationships) and a `Time` column. |
...
| **GUID / UUID** | 16-byte unique identifier. | Propagates NULL. | **Performance Hit.**<br>Import as Text. Very slow for PBI relationships due to size and high uniqueness (cardinality). Use Integers for keys instead. |
