...
| **1:1 (One-to-One)**                        | **Rare.**<br>Usually "Vertical Partitioning" (Split [[Tables|tables]]) or separate tables for security boundaries. | **Unique.**<br>Both sides of the join must be unique.                                  | **Unique.**<br>Both sides of the join must be unique.                              | **Similar to 1:N**<br>If Table A filters Table B, and Table B has a row missing from A, it may result in a blank grouping depending on direction. | **Avoid.**<br>Usually better to merge these into a single table in Power Query for better performance.         |
